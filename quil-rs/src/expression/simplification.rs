...
static RULES: Lazy<Vec<Rewrite>> = Lazy::new(|| {
    vec![
        ...
        // distributivity, associativity, & parenthesis manipulation
        ..rw!("dist r"      ; "(* ?a (+ ?b ?c))"       <=> "(+ (* ?a ?b) (* ?a ?c))"),
        ..rw!("dist l"      ; "(* (+ ?a ?b) ?c)"       <=> "(+ (* ?a ?c) (* ?b ?c))"),
        ..rw!("add assoc"   ; "(+ ?a (+ ?b ?c))"       <=> "(+ (+ ?a ?b) ?c)"),
        ..rw!("mul assoc"   ; "(* ?a (* ?b ?c))"       <=> "(* (* ?a ?b) ?c)"),
        ..rw!("sub paren"   ; "(- ?a (- ?b ?c))"       <=> "(+ (- ?a ?b) ?c)"),
        ..rw!("div paren"   ; "(/ ?a (/ ?b ?c))"       <=> "(* (/ ?a ?b) ?c)"),
        ..rw!("+- paren"    ; "(+ ?a (- ?b ?c))"       <=> "(- (+ ?a ?b) ?c)"),
        ..rw!("-+ paren"    ; "(- ?a (+ ?b ?c))"       <=> "(- (- ?a ?b) ?c)"),
        ..rw!("*/ paren"    ; "(* ?a (/ ?b ?c))"       <=> "(/ (* ?a ?b) ?c)"),
        ..rw!("/* paren"    ; "(/ ?a (* ?b ?c))"       <=> "(/ (/ ?a ?b) ?c)"),
        ..rw!("*- paren r"  ; "(* ?a (- ?b ?c))"       <=> "(- (* ?a ?b) (* ?a ?c))"),
        ..rw!("*- paren l"  ; "(* (- ?a ?b) ?c)"       <=> "(- (* ?a ?c) (* ?b ?c))"),
        ..rw!("/+ paren l"  ; "(/ (+ ?a ?b) ?c)"       <=> "(+ (/ ?a ?c) (/ ?b ?c))"),
        // joining affine expressions
        rw!("affine+++"     ; "(+
                                 (+ (* ?a1 ?x) ?b1)
                                 (+ (* ?a2 ?x) ?b2))"   => "(+
                                                              (* (+ ?a1 ?a2) ?x)
...
mod tests {
    ...
    egg::test_fn! {
          huge_eggspression,
          &RULES,
          runner = egg::Runner::default().with_scheduler(
              ...
              .with_time_limit(core::time::Duration::from_secs(60 * 30)),
          "(/
            (+
              (*
                6.283185307179586
              ...
              0.4345210910722077)
            6.283185307179586)"
            => "(+ -14553.919984548367 (* gamma[0] -0.6379644761225252))"
    }
