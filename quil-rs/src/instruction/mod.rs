...
/// Implement pickling for an instruction.
///
...
/// and so this implementation uses `to_quil` and `Instruction::from_str`,
...
/// ```
///
/// This macro adds the `parse` method to the class to convert a string to an instance,
/// and implements `__reduce__` to return that method along with a string generated by `to_quil`;
/// thus, this pickling implementation requires that the type provides `to_quil(&self) -> String`.
/// Assuming that implementation comes via [`Quil`](quil_rs::quil::Quil),
/// pickling can fail if `to_quil()` contains placeholders.
///
...
macro_rules! impl_pickle_for_instruction {
    ($name: ident) => {
        ...
        impl $name {
            #[staticmethod]
            #[pyo3(name = "parse")]
            fn py_parse<'py>(string: &str, py: Python<'py>) -> PyResult<Bound<'py, Self>> {
...
/// ```
///
/// This will generate `__repr__` for all three types,
/// the `to_quil` methods for `A` and `C`, and the pickle methods for `C`.
macro_rules! impl_instruction {
    // Terminal rule -- an empty list expands to nothing.
    ([]) => {};

    // An identifier with "- quil" leaves off the to_quil impl.
    // That means it only implements `__repr__`, but we can keep it in the instructions list.
    ([$name: ident - quil, $($tail: tt)*]) => {
        impl_repr!($name);
        impl_instruction!([$($tail)*]);
    };

    // An identifier with "- pickle" drops the pickle impl, but still implements to_quil.
    ([$name: ident - pickle, $($tail: tt)*]) => {
        impl_repr!($name);
        impl_to_quil!($name);
        impl_instruction!([$($tail)*]);
    };

    // Implements extract methods for an instruction, then recursively expands the rest of the list.
    ([$name: ident, $($tail: tt)*]) => {
        impl_repr!($name);
        impl_to_quil!($name);
        impl_pickle_for_instruction!($name);
        impl_instruction!([$($tail)*]);
    };
}
...
impl_instruction!([
    Arithmetic,
    ArithmeticOperand - pickle,
    ArithmeticOperator - pickle,
    AttributeValue - pickle,
    BinaryLogic,
    BinaryOperand - pickle,
    BinaryOperator - pickle,
    Calibration,
    CalibrationIdentifier - pickle,
    Call,
    UnresolvedCallArgument,
    Capture,
    CircuitDefinition,
    Comparison,
    ComparisonOperand - pickle,
    Convert,
    Declaration,
    Delay,
    Exchange,
    ExternParameter,
    ExternParameterType - pickle,
    ExternSignature,
    Fence,
    FrameDefinition,
    FrameIdentifier - pickle,
    Gate,
    GateDefinition,
    GateModifier - pickle,
    GateSpecification - pickle,
    Include,
    Instruction,
    Jump,
    JumpUnless,
    ...
    MeasureCalibrationIdentifier - pickle,
    Measurement,
    MemoryReference,
    Move,
    Offset - pickle,
    ...
    PauliSum - quil,
    Pragma,
    PragmaArgument - pickle,
    Pulse,
    Qubit - pickle,
    QubitPlaceholder - quil,
    RawCapture,
    Reset,
    ScalarType - pickle,
    SetFrequency,
    ...
    SwapPhases,
    Target - pickle,
    TargetPlaceholder - quil,
    UnaryLogic,
    UnaryOperator - pickle,
    Vector - pickle,
    Waveform - quil,
    WaveformDefinition,
    WaveformInvocation - pickle,
]);
