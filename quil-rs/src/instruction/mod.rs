...
/// Add a `parse` method to a pyclass by calling `Instruction::from_str`,
/// then attempting to `match` it to the wrapped struct.
///
/// If the `Instruction` variant has a different name than the wrapped type,
/// use `impl_parse_for_instruction($name, $other)` to match against
/// `Instruction::$other` instead of `Instruction::$name`.
macro_rules! impl_parse_for_instruction {
    ($name: ident) => {
        impl_parse_for_instruction!($name, $name);
    };

    ($name: ident, $instrT: ident) => {
        #[pyo3::pymethods]
        impl $name {
            #[staticmethod]
            #[pyo3(name = "parse")]
            fn py_parse(string: &str) -> Result<Self, ParseInstructionError> {
                let instr = Instruction::from_str(string)?;
                match instr {
                    Instruction::$instrT(instr) => Ok(instr),
                    instr => {
                        Err(ParseInstructionError::Parse(
                            format!(
                                concat!("Expected a ", stringify!($name), " but got {:?}"), instr
                                )
                            )
                        )
                    },
...
/// Implement Python's `__reduce__` via a class's `parse` and `to_quil` (Python) methods.
///
/// Normally, Python objects should prefer to implement `__getnewargs__`
/// to provide a tuple of `args` that should be passed to `__new__`;
/// however, implementing that for each individual Instruction would be a pain,
/// and so this implementation uses `to_quil` and `parse`,
/// connected to the Python Pickling machinery via [`__reduce__`][],
/// via a method that, in Python, would look something like this:
///
/// ```python
///     def __reduce__(self) -> tuple[Callable[[str], Self], str]:
///         return self.parse, (self.to_quil(),)
/// ```
///
/// This implements `__reduce__` to return the `parse` method and the string generated by `to_quil`;
/// thus, this pickling implementation requires that the type provides
/// `to_quil(&self) -> String` and `parse(String) -> Self`.
///
...
macro_rules! impl_instruction {
    ...
    ([$name: ident, $($tail: tt)*]) => {
        impl_instruction!(@one $name [+ repr + quil + parse_for_instruction + reduce]);
    ...
    // Note that `reduce` requires the type implements `to_quil` and `parse` `#[pymethods]`.
    (@one $name: ident [+ reduce $($tail: tt)*]) => {
        impl_reduce!($name);
        impl_instruction!(@one $name [$($tail)*]);
    };
    ...
    // There are three variants for the `#[pyo3(name = parse)]` method:
    // - `+ parse`: generates the function just using `$name::from_str`.
    // - `+ parse_for_instruction`: uses `Instruction::from_str` and tries to match `Instruction::name`.
    // - `+ parse_for_instruction(other)`: uses `Instruction::from_str`,
    //      but tries to match against `Instruction::other` instead of `Instruction::name`.
    (@one $name: ident [+ parse_for_instruction($other:ident) $($tail: tt)*]) => {
        impl_parse_for_instruction!($name, $other);
        impl_instruction!(@one $name [$($tail)*]);
    };

    (@one $name: ident [+ parse_for_instruction $($tail: tt)*]) => {
        impl_parse_for_instruction!($name);
        impl_instruction!(@one $name [$($tail)*]);
    };
...
impl_instruction!([
    Arithmetic,
    ArithmeticOperand [repr + quil],
    ArithmeticOperator [repr + quil],
    AttributeValue [repr + quil],
    BinaryLogic,
    BinaryOperand [repr + quil],
    BinaryOperator [repr + quil],
    Calibration [repr + quil + parse_for_instruction(CalibrationDefinition) + reduce],
    CalibrationIdentifier [repr + quil],
    Call,
    UnresolvedCallArgument [repr + quil],
    Capture,
    CircuitDefinition,
    Comparison,
    ComparisonOperand [repr + quil],
    Convert,
    Declaration,
    Delay,
    Exchange,
    ExternParameter [repr + quil],
    ExternParameterType [repr + quil],
    ExternSignature [repr + quil],
    Fence,
    FrameDefinition,
    FrameIdentifier [repr + quil],
    Gate,
    GateDefinition,
    GateModifier [repr + quil],
    GateSpecification [repr + quil],
    Include,
    Instruction [repr + quil + parse + reduce],
    Jump,
    JumpUnless,
    JumpWhen,
    Label,
    ...
    MeasureCalibrationIdentifier [repr + quil],
    Measurement,
    ...
    PauliSum [repr],
    Pragma,
    PragmaArgument [repr + quil],
    Pulse,
    Qubit [repr + quil],
    QubitPlaceholder [repr],
    RawCapture,
    Reset,
    ScalarType [repr + quil],
    SetFrequency,
    SetPhase,
    SetScale,
    Sharing [repr],
    ...
    UnaryLogic,
    UnaryOperator [repr + quil],
    Vector [repr + quil],
    Waveform [repr],
    WaveformDefinition,
    WaveformInvocation [repr + quil],
]);
