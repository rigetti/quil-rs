...
use crate::instruction::{ExternMap, FrameIdentifier, Instruction, InstructionHandler, Target};
...
impl<'a> ScheduledBasicBlock<'a> {
    ...
    pub fn build(
        ...
        let extern_map =
            ExternMap::try_from(program.extern_pragma_map.clone()).map_err(|(pragma, _)| {
                ScheduleError {
        ...
        for (index, &instruction) in basic_block.instructions().iter().enumerate() {
            ...
            let accesses = custom_handler
                .memory_accesses(instruction, &extern_map)
